...
class Post {
	...
	public function submitPost($body, $user_to, $imageName) {
		...
		if($check_empty != "") {
			...
			//Trending words list to be removed
			...
			$stopWords = "a about above across after again against all almost alone along already
             ...
             hate sleepy reason for some little yes bye choose hello guy guys forward looking";
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if ($user_logged_obj->isFriend($added_by)) {
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<button class='delete_button btn-danger fa fa-trash-o' id='post$id'></button>";
				else
					$delete_button = "";
					...
					$str .= "<div class='status_post' onClick='javascript:toggle$id()'>
								...
								<div class='posted_by' style='color:#ACACAC; margin-bottom: 5px;'>
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<button class='delete_button btn-danger fa fa-trash-o fw' id='post$id'></button>";
				else
					$delete_button = "";
